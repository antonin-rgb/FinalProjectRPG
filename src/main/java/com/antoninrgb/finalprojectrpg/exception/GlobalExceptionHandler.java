...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleGeneral(Exception ex) {
        ApiError error = new ApiError("Unexpected error: " + ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR.value(), LocalDateTime.now());
        return new ResponseEntity<>(error, HttpStatus.INTERNAL_SERVER_ERROR);
    }

    // complete with: throw new ResourceNotFoundException("Enemy not found: " + name);
}
